...
declare -A burnin1=([SALMA_kept/species_level/1-Full]=50 [SALMA_smoothed/species_level/1-Full]=50 \
...
do
	...
	do
		### Full ###
		python ~/PyRate/PyRate.py -combLogRJ ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/pyrate_mcmc_logs -tag KEEP -b ${burnin1[$s/$ss/1-Full]}
		mkdir -p ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/combined_logs
		mv ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/pyrate_mcmc_logs/combined_* ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/combined_logs
		#Plot RTT
		python ~/PyRate/PyRate.py -plotRJ ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/combined_logs -b ${burnin1[$s/$ss/1-Full]}
		#Plot LTT
		mkdir -p ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/LTT/
		python ~/PyRate/PyRate.py -ginput ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/pyrate_mcmc_logs -tag KEEP_ -b ${burnin1[$s/$ss/1-Full]}
		mv ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/pyrate_mcmc_logs/*_se_est.txt ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/LTT/
		mv ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/combined_logs/*_LTT.r ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/LTT/
		#Diversity through time in the right format for BDS and MBD
		python ~/PyRate/PyRate.py -d ../results_EXTENDED/$s/$ss/1-Full/MH_sampler/LTT/*_se_est.txt -ltt 1
