...
do
	...
	do
		...
		## "Order-level" (Phylogenetic subdivisions)
		for O in Notoungulata Other_SANUs Rodentia Xenarthra Metatheria
		do
			#Combine logs
			python ~/PyRate/PyRate.py -combLogRJ ../results/$s/$ss/6-Order_level/$O/pyrate_mcmc_logs -tag _KEEP -b ${burnin1[$s/$ss/6-Order_level/$O]}
			mkdir -p ../results/$s/$ss/6-Order_level/$O/combined_logs
			mv ../results/$s/$ss/6-Order_level/$O/pyrate_mcmc_logs/combined_* ../results/$s/$ss/6-Order_level/$O/combined_logs
			#Plot RTT
			...
			#Diversity through time in the right format for MBD
			python ~/PyRate/PyRate.py -d ../results/$s/$ss/6-Order_level/$O/LTT/*_se_est.txt -ltt 1
		done
	done
done
