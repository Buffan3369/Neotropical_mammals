...
do
	...
	do
		if [[ "$s" == SALMA_kept && "$ss" == species_level ]]; then
		    continue
		fi
		for ana in 2-Singleton 3-Spatially_scaled
		do
	...
	#		Rscript ~/Documents/GitHub/CorsaiR/R/plot_ess.r "$PWD"/$dir/pyrate_mcmc_logs/ESS_summary.txt "$PWD"/$dir/pyrate_mcmc_logs/ESS_plot.png
			...
			python ~/Documents/GitHub/CorsaiR/Python/parse_Q_rates.py -q_dir $dir/Q_SHIFTS -bin_dir ../data_2023/Extended_EO_stages.txt -out $dir/Q_SHIFTS/Parsed_Q_rates.txt
		done
	done
done
