...
j <- 0
for(k in 1:2){
  rtt_tbl <- extract_rtt(path = paste0(paths[k], "combined_logs/RTT_plots.r"), ana = "RJMCMC")
  ...
  ltt <- list.files(paste0(paths[k], "LTT/"), pattern = "se_est_ltt.txt")[[1]]
  ltt_tbl <- read.table(paste0(paths[k], "LTT/", ltt), header = TRUE)
  ltt_tbl <- ltt_tbl %>%
