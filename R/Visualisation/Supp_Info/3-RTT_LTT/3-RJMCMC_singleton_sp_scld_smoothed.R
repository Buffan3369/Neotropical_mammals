################################################################################
# Name: 3-RJMCMC_singleton_sp_scld_smoothed.R
# Author: Lucas Buffan
...
for(ana in c("2-Singleton", "3-Spatially_scaled")){
  paths <- c(paste0("./results_EXTENDED/SALMA_smoothed/genus_level/", ana, "/"),
             paste0("./results_EXTENDED/SALMA_smoothed/species_level/", ana, "/"))
  i <- 0
  j <- 0
  plot_list1 <- list()
  for(pth in paths){
  ...
  ggsave(paste0("./figures/supp_figs/RTT_LTT/RTT_LTT_", ana, "_smoothed.pdf"),
         plot = fig1,
         height = 200,
         width = 400,
         units = "mm")
