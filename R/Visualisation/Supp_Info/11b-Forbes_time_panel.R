...
# compute corrected Forbes similarities through time across the five groups
FI <- data.frame(age = seq(52, 24, -0.1))
forb_idx <- function(t, cls){
...
for(cls in keys(tax_dict)){
  FI <- FI %>% add_column(sapply(X = seq(52, 24, -0.1), FUN = function(x){return(forb_idx(t = x, cls = cls))}))
  colnames(FI) <- c(colnames(FI)[1:length(FI)-1], cls)
}
...
forb_plot <- FI %>% 
  pivot_longer(cols = keys(tax_dict),
               names_to = "group",
               values_to = "forbes") %>% 
  ggplot(aes(x = age, y = forbes)) +
  scale_x_reverse() +
  geom_line() +
  labs(x = "Time (Ma)", y = "corrected Forbes index") +
  geom_vline(xintercept = 33.9, linetype="dashed", color = "red", linewidth = 0.8) +
  annotate(geom = "text", x = 32, y = 0.95, label = "EOT", size = 5, colour = "red") +
...
ggsave("./figures/supp_figs/forbes_through_time.pdf", plot = forb_plot, height = 200, width = 300, units = "mm")
